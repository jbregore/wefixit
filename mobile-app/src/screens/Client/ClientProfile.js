...
const ClientProfile = ({ navigation }) => {
  ...
  const loadData = () => {
    fetch('http://192.168.42.241/wefixit/backend/api/users/user_fetch_self.php', {
      ...
      .then((responseJson) => {
        ...
        fetch('http://192.168.42.241/wefixit/backend/api/appointments/client_fetch_profile_appointments.php', {
          method: 'POST',
          ...
          .then((responseJson) => {
            ...
            for (let row of result) {
              ...
              //JOB POST INFO
              fetch('http://192.168.42.241/wefixit/backend/api/jobs/fetch_edit_job.php?id=' + row.jobpost_id, {
                method: 'GET',
                headers: {
                  //Header Defination
                  'Accept': 'application/json',
  ...
  useEffect(() => {
    ...
    for (let row of appointmentList) {
      //FEEDBACKS INFO
      fetch('http://192.168.42.241/wefixit/backend/api/appointments/fetch_feedbacks.php?id=' + row.id, {
        method: 'GET',
        headers: {
          //Header Defination
          'Accept': 'application/json',
    ...
    for (let row of appointmentList) {
      console.log(row.id)
      //FEEDBACKS PHOTOS
      fetch('http://192.168.42.241/wefixit/backend/api/appointments/fetch_photos.php', {
        method: 'POST',
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.screen}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} refreshControl={
          ...
          {loadingInfo === false ? (
            <>
              <View style={{ flexDirection: "row", marginTop: 25, marginBottom: 25, marginLeft: 10 }}>
                <Avatar.Image
                  size={60}
                  source={{
